import type { CommitActivityWeek, Contributor, LanguageBreakdown, RepoInfo, RepoReview } from './types'
...
async function get<T>(path: string): Promise<T> {
  ...
  if (res.status === 404) throw new GitHubApiError('Repository not found. Check the owner/name and try again.', 404)
  if (res.status === 403) {
