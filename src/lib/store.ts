...
interface ReviewState {
  ...
  reset: () => void;
  setReview: (review: RepoReview) => void;
  setUser: (profile: UserProfile, repos: UserRepoSummary[]) => void;
}
...
export const useReviewStore = create<ReviewState>((set, get) => ({
  ...
  setUser: (profile: UserProfile, repos: UserRepoSummary[]) => {
    set({
      userProfile: profile,
      userRepos: repos,
      mode: "user",
  ...
  search: async (input: string) => {
    ...
    if (parsedRepo) {
      ...
      try {
        ...
        // Save to cache
        saveCache(`repo:${parsedRepo.owner}/${parsedRepo.repo}`, review);
        ...
        const message =
          err instanceof GitHubApiError
            ? err.message
            : "Something went wrong fetching that repo.";
    ...
    if (username) {
      ...
      try {
        ...
        // Save to cache
        saveCache(`user:${username}`, { profile, repos });
        setUserQueryParam(profile.login);
      } catch (err) {
        const message =
          err instanceof GitHubApiError
            ? err.message
            : "Something went wrong fetching that user.";
        set({
          loading: false,
          error: message,
          userProfile: null,
    ...
    set({
      error: 'Enter "owner/repo", a github.com URL, or just a username.',
      review: null,
    });
  },
  ...
  reset: () =>
    set({ review: null, userProfile: null, userRepos: null, error: null }),
}));
